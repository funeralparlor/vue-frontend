...
const loadTrashedYearLevels = async (page = 1) => {
  try {
    ...
    const response = await YearLevelService.getTrash(params);
    state.trashedYearLevels = response.data.data;
    state.currentPage = response.data.page;
    state.totalPages = response.data.pages;
    state.totalItems = response.data.total;
  } catch (error) {
    handleError(error, 'Failed to load trashed year level');
  } finally {
...
const changePage = (page) => {
  if (page >= 1 && page <= state.totalPages && page !== state.currentPage) {
    loadTrashedCampues(page);
  }
};

// Restore YearLevels with confirmation
const restoreYearLevel = async (id) => {
  if (!confirm('Are you sure you want to restore this year level?')) return;
  
  try {
    state.isRestoring = true;
    await YearLevelService.restore(id);
    showAlert('Year Level restored successfully', 'success');
    await loadTrashedYearLevels(state.currentPage);
  } catch (error) {
    handleError(error, 'Failed to restore year level');
  } finally {
...
const permanentlyDelete = async (id) => {
  if (!confirm('Are you sure you want to permanently delete this year level? This action cannot be undone.')) return;
  
  try {
    state.isDeleting = true;
    await YearLevelService.forceDelete(id);
    showAlert('Year Level permanently deleted', 'success');
    
    // If this was the last item on the page and not the first page, go to previous page
    if (state.trashedYearLevels.length === 1 && state.currentPage > 1) {
      await loadTrashedYearLevels(state.currentPage - 1);
    } else {
      await loadTrashedYearLevels(state.currentPage);
    }
  } catch (error) {
    handleError(error, 'Failed to permanently delete year level');
  } finally {
...
const changePerPage = (value) => {
  state.perPage = value;
  loadTrashedYearLevels(1); // Reset to first page when changing items per page
};
...
// Initial data fetch
onMounted(() => loadTrashedYearLevels());
</script>

<template>
  <MainLayout pageTitle="Year Level Trash">
    <div class="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow">
      <!-- Header section -->
      <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900">Year Level Trash Bin</h2>
        <p class="mt-1 text-sm text-gray-600">
          View, restore, or permanently delete trashed Year Level records
        </p>
      </div>
      ...
      <div v-if="state.isLoading" class="text-center p-12">
        <p class="text-gray-600 animate-pulse">Loading trashed year levels...</p>
      </div>

      <!-- Error state -->
      <div v-else-if="state.error" class="bg-red-50 p-4 rounded-md mb-6">
        <p class="text-red-700">{{ state.error }}</p>
        <button @click="loadTrashedYearLevels()" class="mt-2 text-red-600 underline">Try again</button>
      </div>

      <!-- Empty state -->
      <div v-else-if="state.trashedYearLevels.length === 0" class="text-center p-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500">No year level found in trash</p>
      </div>
      ...
      <div v-else class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="yearlevel in state.trashedYearLevels" :key="yearlevel.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ new Date(yearlevel.deleted_at).toLocaleString() }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-center space-x-2">
                <button 
                  @click="restoreYearLevel(yearlevel.id)" 
                ...
                <button 
                  @click="permanentlyDelete(yearlevel.id)" 

...
const loadTrashedCampuses = async (page = 1) => {
  try {
    ...
    const response = await CampusService.getTrash(params);
    state.trashedCampuses = response.data.data;
    state.currentPage = response.data.page;
    state.totalPages = response.data.pages;
    state.totalItems = response.data.total;
  } catch (error) {
    handleError(error, 'Failed to load trashed campuses');
  } finally {
...
const restoreCampus = async (id) => {
  if (!confirm('Are you sure you want to restore this campus?')) return;
  
  try {
    state.isRestoring = true;
    await CampusService.restore(id);
    showAlert('Campus restored successfully', 'success');
    await loadTrashedCampuses(state.currentPage);
  } catch (error) {
    handleError(error, 'Failed to restore campus');
  } finally {
...
const permanentlyDelete = async (id) => {
  if (!confirm('Are you sure you want to permanently delete this campus? This action cannot be undone.')) return;
  
  try {
    state.isDeleting = true;
    await CampusService.forceDelete(id);
    showAlert('Campus permanently deleted', 'success');
    
    // If this was the last item on the page and not the first page, go to previous page
    if (state.trashedCampuses.length === 1 && state.currentPage > 1) {
      await loadTrashedCampuses(state.currentPage - 1);
    } else {
      await loadTrashedCampuses(state.currentPage);
    }
  } catch (error) {
    handleError(error, 'Failed to permanently delete campus');
  } finally {
...
const changePerPage = (value) => {
  state.perPage = value;
  loadTrashedCampuses(1); // Reset to first page when changing items per page
};
...
// Initial data fetch
onMounted(() => loadTrashedCampuses());
</script>

<template>
  <MainLayout pageTitle="Campus Trash">
    <div class="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow">
      <!-- Header section -->
      <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900">Campus Trash Bin</h2>
        <p class="mt-1 text-sm text-gray-600">
          View, restore, or permanently delete trashed Campus records
        </p>
      </div>
      ...
      <div v-if="state.isLoading" class="text-center p-12">
        <p class="text-gray-600 animate-pulse">Loading trashed campuses...</p>
      </div>

      <!-- Error state -->
      <div v-else-if="state.error" class="bg-red-50 p-4 rounded-md mb-6">
        <p class="text-red-700">{{ state.error }}</p>
        <button @click="loadTrashedCampuses()" class="mt-2 text-red-600 underline">Try again</button>
      </div>

      <!-- Empty state -->
      <div v-else-if="state.trashedCampuses.length === 0" class="text-center p-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500">No campuses found in trash</p>
      </div>
      ...
      <div v-else class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="campus in state.trashedCampuses" :key="campus.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ new Date(campus.deleted_at).toLocaleString() }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-center space-x-2">
                <button 
                  @click="restoreCampus(campus.id)" 
                ...
                <button 
                  @click="permanentlyDelete(campus.id)" 

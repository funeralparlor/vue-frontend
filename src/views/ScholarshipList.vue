...
// Load scholarships with error handling
...
const submitForm = async () => {
  ...
  try {
    if (modal.formData.id) {
      // Update existing
      await ScholarshipService.update(modal.formData.id, modal.formData);
      showAlert('Scholarship updated successfully', 'success');
    } else {
      // Create new
      await ScholarshipService.create(modal.formData);
      showAlert('Scholarship created successfully', 'success');
    }
    
    // Close modal and refresh list
    modal.show = false;
    loadScholarships(state.currentPage);
  } catch (error) {
    if (error.response?.data?.errors) {
      modal.formErrors = error.response.data.errors;
    } else {
      handleError(error, 'Failed to save scholarship');
    }
  } finally {
...
const confirmDelete = async (id) => {
  ...
  try {
    ...
    if (state.scholarships.length === 1 && state.currentPage > 1) {
      await loadScholarships(state.currentPage - 1);
    } else {
      await loadScholarships(state.currentPage);
    }
  } catch (error) {
    handleError(error, 'Failed to delete scholarship');
  } finally {
...
const changePerPage = (value) => {
  state.perPage = value;
  loadScholarships(1); // Reset to first page when changing items per page
};
...
watch(() => state.search, (newValue) => {
  ...
  searchTimeout = setTimeout(() => {
    loadScholarships(1); // Reset to first page when searching
  }, 500);
});
...
<template>
  <MainLayout pageTitle="Scholarships">
    <div class="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow">
      <!-- Header section -->
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          <h2 class="text-2xl font-semibold text-gray-900">Manage Scholarships</h2>
          <p class="mt-1 text-sm text-gray-600">
            Create, view, edit and delete scholarship programs.
          </p>
        </div>
        <button 
          @click="openAddModal" 
      ...
      <div v-if="state.isLoading" class="text-center p-12">
        <p class="text-gray-600 animate-pulse">Loading scholarships...</p>
      </div>

      <!-- Error state -->
      <div v-else-if="state.error" class="bg-red-50 p-4 rounded-md mb-6">
        <p class="text-red-700">{{ state.error }}</p>
        <button @click="loadScholarships()" class="mt-2 text-red-600 underline">Try again</button>
      </div>
      
      <!-- Empty state -->
      <div v-else-if="state.scholarships.length === 0" class="text-center py-12 bg-gray-50 rounded-lg">
        <p class="text-gray-600 mb-4">No scholarships found</p>
        <button 
          @click="openAddModal" 
      ...
      <div v-else class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="scholarship in state.scholarships" :key="scholarship.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <span 
                  :class="{'bg-green-100 text-green-800': scholarship.active, 'bg-red-100 text-red-800': !scholarship.active}"
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-center space-x-2">
                <button 
                  @click="openEditModal(scholarship)" 
                ...
                <button 
                  @click="confirmDelete(scholarship.id)" 
